"""v35 on Colab — v23's exact architecture, testing whether up-weighting the intensity loss term
fixes v23/v34's weak vmax/pressure/RMW forecasting, per the user's observation and the loss-term
audit that motivated this run.

    !wget -q -O /content/v35.py https://raw.githubusercontent.com/yu314-coder/typhoon-predict/<SHA>/colab_v40_train.py
    import os; os.environ["V35_SEEDS"]="5"; exec(open('/content/v35.py').read())

WHY. v23/v34's combined loss is `track_loss(4 weighted terms) + int_loss(1 term, all 15
intensity channels) [+ 0.3*flow_loss for the CoT steering head]`. Counting terms, track/steering
gets ~5x the additive weight intensity gets, and this ratio was inherited unchanged from v17 and
never revisited across the whole v21-v34 line, since every version in that line targeted track.
Empirically: vmax correlation with reality falls from ~0.98 at 6h to ~0.5 by 96-120h (WP+EP
aggregate), and on Typhoon Tip (1979) the mean-forecast bias grows to -28 to -30 kt by 120h --
consistent with an under-weighted intensity loss letting the model hedge toward a moderate
forecast rather than committing to the tails.

WHAT CHANGES. Nothing architectural -- v35 IS v23 (TrackFormerHist(V21)), same forward, same
inputs. The ONLY change is `total_loss`: `track_loss(s,tn,m) + W_INT * int_loss(s,ls,tn,m) +
W_FLOW * flow_loss`, with W_INT a tunable multiplier (default 3.0, roughly balancing intensity
against track's ~4-term weight) instead of the implicit 1.0 baked into v17's `total_loss`. Setting
W_INT=1.0 exactly reproduces v23's original loss -- this is the built-in ablation/sanity check,
not a separate script.

EVALUATION. Reports BOTH track (the metric every other version in this line optimized) and
vmax/pressure/RMW MAE (the metric this run targets), on the same WP+EP 2020+ full-horizon test
set v23/v34 were scored on, so the tradeoff -- if any -- is visible in one place.
"""
...
N_SEEDS = int(os.environ.get("V35_SEEDS", "5"))
W_FLOW = float(os.environ.get("W_FLOW", "0.3"))
W_INT = float(os.environ.get("V35_WINT", "3.0"))    # 1.0 exactly reproduces v23's original loss
USE_HIST = int(os.environ.get("V28_USE_HIST", "1"))     # 0 = ablation, v21 with the same code path
TAG = os.environ.get("V35_TAG", "v35")
KM6H = 6 * 3600 / 1000.0
...
def total_loss(s, ls, fp, tgt, m, fl, fm):
    # v17's own total_loss = track_loss(s,tn,m) + int_loss(s,ls,tn,m), unweighted (W_INT=1
    # reproduces it exactly). Here int_loss gets its own multiplier.
    tn = tgt / TARGET_SCALE
...
def intensity_mae(P):
    T = target[wpep]; K = target_mask[wpep]
    out = {}
    for ci, nm in [(2, "vmax"), (3, "pressure"), (4, "rmw")]:
        vals = np.abs(P[:, :, ci] - T[:, :, ci])[K[:, :, ci]]
        out[nm] = float(vals.mean()) if K[:, :, ci].any() else None
    return out
...
print("  v23 (W_INT=1, the bar for THIS run)  track 434.96 km | vmax 16.43 kt | pres 12.35 hPa | rmw 11.00 nm")
for i, c in enumerate(CK):
    print(f"  {TAG} seed{i}  {track_err([load_m(c)]):.2f} km", flush=True)
e = track_err(MS)
Pfull = full_predict(MS)
im = intensity_mae(Pfull)
print(f"\n  {TAG} ENSEMBLE ({len(MS)} seeds)  track {e:.2f} km | vmax {im['vmax']:.2f} kt | "
      f"pres {im['pressure']:.2f} hPa | rmw {im['rmw']:.2f} nm")
print(f"  vs v23 (W_INT=1): track {e - 434.96:+.2f} km | vmax {im['vmax'] - 16.43:+.2f} kt | "
      f"pres {im['pressure'] - 12.35:+.2f} hPa | rmw {im['rmw'] - 11.00:+.2f} nm", flush=True)
print(f"  W_INT={W_INT} -- if vmax/pres/rmw improved and track got only slightly worse, the loss"
      "\n  rebalance is a real, cheap win; if track got much worse for a small intensity gain, it's"
      "\n  a genuine tradeoff, not a free lunch.", flush=True)
json.dump({TAG: {"track": e, **im}, "w_int": W_INT, "use_hist": USE_HIST, "n_seeds": len(MS)},
