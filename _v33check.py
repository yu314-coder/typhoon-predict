"""Local (CPU) init-assertion check for v33 -- IDENTICAL architecture to v31/v32 (TrackFormerLand),
only A_MAX changed (1.5 -> 1.0) and land_weight() is now STORM-normalized instead of per-window
(checked separately below, since it's the actual new code in v33; the architecture checks are
unchanged from _v31/_v32check.py on purpose -- v32's failure was a training-recipe problem, not
an architecture problem, see colab_v38_train.py's docstring).
Sources every class from LOCAL files instead of urllib/GitHub -- same pattern as _v29/_v31/_v32check.py.
Verifies:
  1. land off  -> v33 output EXACTLY equals v23's (max diff 0)
  ...
  5. state-dict remap loads v23 weights into v33 with only the new `land.*` keys missing
  6. land_weight() (storm-normalized) gives every distinct storm in a bucket the SAME total
     sampling mass regardless of window count, and hits the targeted ~15%/~7% effective share
"""
...
assert _d1 < 1e-5, f"v33 does not reduce to v23 at init: {_d1}"
print(f"OK: land OFF max|v33 - v23| = {_d1:.2e} (v33 starts as exactly v23)")
...
print(f"OK: land ON  max|v33 - v23| = {_d2:.2e} (land path is live)")
...
# ---- land_weight(): the actual new code in v33. STORM-normalized -- every distinct storm in a
# bucket gets the SAME total sampling mass (PER_STORM_W), split evenly across however many of its
# windows fall in that bucket, so a storm with many correlated near-land windows can no longer
# dominate the sampler the way it could under v32's per-window weighting.
BLA = z["base_lat"].astype("float64"); BLO = z["base_lon"].astype("float64") % 360
tr_idx = G["tr_idx"]


def land_weight(idx, chunk=4000):
    """Chunked for the same memory reason as v32 -- land_features_np broadcasts every row against
    all 12,660 land cells at once; the full ~153k-row training set in one shot allocates tens of
    GB of intermediates and OOM'd Colab's High-RAM runtime."""
    n = len(idx)
...
FLAT = NEAR & ~MTN
n_flat_near = int(FLAT.sum()); n_mtn = int(MTN.sum()); n_base = int((~NEAR).sum())
print(f"  baseline (weight=1.0):        {n_base:,} ({100*n_base/len(tr_idx):.1f}%)")
print(f"  flat near-land:                {n_flat_near:,} ({100*n_flat_near/len(tr_idx):.2f}%), "
      f"{len(np.unique(sid[tr_idx][FLAT]))} distinct storms")
print(f"  mountainous near-land:         {n_mtn:,} ({100*n_mtn/len(tr_idx):.2f}%), "
      f"{len(np.unique(sid[tr_idx][MTN]))} distinct storms")
...
# per-storm total mass check: every distinct storm in a bucket must sum to the SAME total weight
# (PER_STORM_W), regardless of how many windows it contributed -- this is the actual v33 fix.
sid_tr = sid[tr_idx]
for bucket_mask, per_storm_w, name in ((MTN, PER_STORM_MTN, "mountainous"), (FLAT, PER_STORM_FLAT, "flat")):
    storms = sid_tr[bucket_mask]; wb = W[bucket_mask]
    totals = {}
    for s, wv in zip(storms, wb):
        totals[s] = totals.get(s, 0.0) + wv
    vals = np.array(list(totals.values()))
    assert np.allclose(vals, per_storm_w, rtol=1e-6), \
        f"{name}: storm totals are not uniform (min {vals.min():.3f} max {vals.max():.3f}, expected {per_storm_w})"
    print(f"  OK: every one of {len(vals)} distinct {name} storms contributes exactly "
          f"{per_storm_w:.0f} total weight mass regardless of window count")

eff_share_mtn = W[MTN].sum() / W.sum(); eff_share_flat = W[FLAT].sum() / W.sum()
print(f"  effective per-epoch share: mountainous {100*eff_share_mtn:.1f}%  flat {100*eff_share_flat:.1f}%  "
      f"(v32 was ~48%/~9.5%, target ~15%/~7%)")
assert 0.08 < eff_share_mtn < 0.25, f"mountainous effective share {100*eff_share_mtn:.1f}% is outside the intended ~15% range"
print("OK: land_weight() is storm-normalized and the dosage lands in the intended, much more")
print("    conservative range compared to v32")

print("\nALL CHECKS PASSED -- v33 (TrackFormerLand + storm-normalized near-land training) is ready for Colab")
