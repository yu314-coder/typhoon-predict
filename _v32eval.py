"""v32 (oversampled near-land training) vs v31 (uniform training) vs v23, on the WP-only test set,
broken down by land proximity/terrain exactly like _landtest_cross.py's Q1/Q3 -- since that's the
test that actually confirms or kills the oversampling fix, not the aggregate km number alone.

RESULT ALREADY KNOWN from the Colab print: v32 ENSEMBLE full WP+EP 460.48 km, WP-only 495.69 km --
worse than v23 (434.96) by +25.52 km, and worse than v31 (443.07) too. This script checks whether
the extra harm is concentrated in the mountainous-near-land subset it was supposed to fix (i.e.
some benefit there, catastrophic overfitting elsewhere) or whether it's uniformly worse everywhere
(i.e. the oversampling destabilized training broadly, not a targeted trade-off).
"""
...
print(f"v23: {len(MS['v23'])} | v31: {len(MS['v31'])} | v32: {len(MS['v32'])} seeds")
...
E = {tag: errors(tag, TE) for tag in ("v23", "v31", "v32")}
AC = {tag: along_cross(TE, E[tag]) for tag in ("v23", "v31", "v32")}

LEADS = (0, 3, 7, 11, 15, 19)  # 6,24,48,72,96,120h
print(f"\n{'bucket':>20s} {'lead_h':>7s} | {'v23 along':>10s} {'v31 along':>10s} {'v32 along':>10s} | "
      f"{'v23 cross':>10s} {'v31 cross':>10s} {'v32 cross':>10s}")
for name, mask in (("open_ocean", ~near_t), ("flat_near_land", flat_t), ("mountainous_near_land", mountain_t)):
    ...
    for L in LEADS:
        row = [name, 6 * (L + 1)]
        vals = []
        for tag in ("v23", "v31", "v32"):
            al, cr = AC[tag]
            vals.append(float(np.sqrt((al[mask, L] ** 2)).mean()))
        for tag in ("v23", "v31", "v32"):
            al, cr = AC[tag]
            vals.append(float(np.sqrt((cr[mask, L] ** 2)).mean()))
        print(f"{name:>20s} {6*(L+1):7d} | {vals[0]:10.1f} {vals[1]:10.1f} {vals[2]:10.1f} | "
              f"{vals[3]:10.1f} {vals[4]:10.1f} {vals[5]:10.1f}")
...
print(f"\noverall track RMS error (km) by bucket, v23 vs v31 vs v32:")
for name, mask in (("open_ocean", ~near_t), ("flat_near_land", flat_t), ("mountainous_near_land", mountain_t)):
    if mask.sum() == 0:
        continue
    for L, lbl in ((7, "48h"), (19, "120h")):
        ...
        for tag in ("v23", "v31", "v32"):
            e = E[tag][mask, L]
            row.append(float(np.sqrt((e ** 2).sum(-1)).mean()))
        print(f"  {name:22s} {lbl:5s} | v23 {row[0]:7.1f}  v31 {row[1]:7.1f}  v32 {row[2]:7.1f}  "
              f"| v32-v23 {row[2]-row[0]:+7.1f}  v32-v31 {row[2]-row[1]:+7.1f}")
