...
d = np.load(f"{TD}/per_storm_v20_v17.npz", allow_pickle=True)
...
<title>Where deep-layer steering helps — v20 vs v17</title>
<style>
...
<div class="wrap">
 <div>
  <div class="eyebrow">TrackFormer · v20 minus v17, per storm</div>
  <h1>Deep-layer steering doesn't help everywhere — it trades</h1>
 </div>
 <p class="lede">Every 2020+ WP/EP test storm, drawn once and coloured by which model forecast it
 better. <strong>Blue</strong> means the deep-layer-mean model (v20) beat the single-level 500&nbsp;hPa
 model (v17) on that storm; <strong>red</strong> means it lost. Thick lines are the 18 storms where
 ...
 <div class="stats">
  ...
  <div class="stat"><b>462.8</b><span>v17 mean error, km (5 seeds)</span></div>
  <div class="stat"><b>{nb}/144</b><span>storms v20 wins ({100*nb/144:.0f}%)</span></div>
  <div class="stat"><b>+0.5</b><span>median per-storm change, km</span></div>
 </div>
 <footer>
  <p><strong>The average gain is real arithmetic but not a broad improvement.</strong> v20 is 10.35&nbsp;km
  better on average, yet wins on only {nb} of 144 storms and the median storm is half a kilometre
  <em>worse</em>. A paired-storm bootstrap over 2000 resamples puts the difference at
  &minus;10.35&nbsp;km with a 95% CI of [&minus;22.54, +0.82] &mdash; it misses significance by
  0.82&nbsp;km.</p>
  <p>Mean track latitude is {w_lat:.1f}°N for the storms v20 wins and {l_lat:.1f}°N for the ones it
  loses; peak latitude averages {peak_w:.1f}°N against {peak_l:.1f}°N. If those numbers were far
  apart, the wins would be the recurving storms where the deep-layer flow genuinely departs from
  500&nbsp;hPa, and the fix would be to use deep-layer steering only there. Read the map before
  trusting that story: interleaved colours mean variance, spatial clustering means physics.</p>
...
open("paper/v20_v17_winloss_map.html", "w").write(HTML)
print(f"wrote paper/v20_v17_winloss_map.html ({len(HTML)/1000:.0f} KB)")
