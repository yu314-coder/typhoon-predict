...
CKPT = os.environ.get("TRACK_CKPT", "track_build/track_v11_best.pt")
...
print(f"windows {len(track)} | dims kin {KIN_DIM} thermo {THERMO_DIM} env {ENV_DIM}")
...
class TrackFormerV9(nn.Module):
    ...
    def forward(self, track, v0):
        ...
        motion = (self.rho.view(1, self.leads, 1) * v0.unsqueeze(1)) / 100.0 + self.track_res(h_track)
...
def _mag(e, n): return torch.sqrt(e * e + n * n + 1e-8)   # differentiable at 0 (torch.hypot grad is NaN there)
def speed_loss(s, tn, m):
    # explicit per-step moving-speed magnitude error (the model under-reacts to speed changes)
    ps = _mag(s[..., 0], s[..., 1]); ts = _mag(tn[..., 0], tn[..., 1]); mm = m[..., 0]
    return (F.smooth_l1_loss(ps, ts, reduction="none") * mm).sum() / mm.sum().clamp(min=1)
def dir_loss(s, tn, m):
    # explicit moving-DIRECTION (angle) error via cosine distance, independent of speed
    pe, pn, te, tne = s[..., 0], s[..., 1], tn[..., 0], tn[..., 1]; mm = m[..., 0]
    cos = ((pe * te + pn * tne) / (_mag(pe, pn) * _mag(te, tne))).clamp(-1.0, 1.0)
    return ((1.0 - cos) * mm).sum() / mm.sum().clamp(min=1)
def total_loss(s, logs, tgt, m):
    # v11: prioritize wind/pressure/radii (x1.8) + explicit moving-speed + moving-direction terms; track anchored
    tn = tgt / TARGET_SCALE
    return (track_loss(s, tn, m) + 1.8 * int_loss(s, logs, tn, m)
            + 0.5 * speed_loss(s, tn, m) + 0.6 * dir_loss(s, tn, m))
...
def metrics(idx):
    ...
    # wind radii, split by threshold. R34 = the LEVEL-7 gale radius (七级风圈)
    for lo, hi, nm in [(5, 9, "R34_level7_mae_km"), (9, 13, "R50_mae_km"), (13, 17, "R64_mae_km")]:
        rm = M[..., lo:hi] > 0.5
        out[nm] = round(float(np.abs(P[..., lo:hi] - T[..., lo:hi])[rm].mean()), 2) if rm.any() else None
    # moving speed (per-step km/6h) and moving direction (angle, deg)
    ps = np.hypot(P[..., 0], P[..., 1]); ts = np.hypot(T[..., 0], T[..., 1]); sm = M[..., 0] > 0.5
    out["moving_speed_mae_km6h"] = round(float(np.abs(ps[sm] - ts[sm]).mean()), 2)
    cos = (P[..., 0] * T[..., 0] + P[..., 1] * T[..., 1]) / (ps * ts + 1e-6)
    ang = np.degrees(np.arccos(np.clip(cos, -1, 1)))
    out["moving_dir_mae_deg"] = round(float(ang[sm].mean()), 2)
    return out
