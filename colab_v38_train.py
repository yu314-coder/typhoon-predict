"""v33 on Colab -- v31/v32's EXACT architecture (LandDrag/TrackFormerLand, unchanged), retrained
a THIRD time with a storm-level (not window-level) oversampling split, after v32's window-level
oversampling backfired badly.

    !wget -q -O /content/v33.py https://raw.githubusercontent.com/yu314-coder/typhoon-predict/<SHA>/colab_v38_train.py
    import os; os.environ["V33_SEEDS"]="5"; exec(open('/content/v33.py').read())

WHY v31 LOST to v23 everywhere it was tested (see land-interaction-lever memory), and why the
land-drag effect is real but narrow (mountainous coastline specifically, not flat, per
_landtest_cross.py's terrain-height breakdown): see colab_v37_train.py's docstring, unchanged
reasoning, not repeated here.

WHY v32 (window-level oversampling, near-land 6x / mountainous 21x, effective per-epoch share
47.9%) made things WORSE than v31 and v23 in EVERY bucket, worst exactly in the targeted
mountainous-near-land bucket (+203.6 km at 120h vs v23): investigated with a storm-count audit
before building this. Two candidate explanations were on the table -- (a) oversampling by WINDOW
count when a storm contributes many correlated windows means "10x oversampled" is really "see the
same few storms 10x more," overfitting their idiosyncrasies; (b) the aggregate DOSAGE was simply
too large (47.9% effective share for a ~4.8%-real phenomenon), starving the open-ocean majority of
training exposure within the same early-stopping budget (the OPEN-OCEAN bucket got worse too,
which a purely-storm-level explanation doesn't obviously predict).

The storm-count audit found the mountainous-near-land bucket actually has GOOD storm diversity:
640 distinct storms contribute 7,380 training windows (11.5 windows/storm average), and the top 5
storms by window count are only 4.0% of the bucket -- not dominated by a handful of storms. So (a)
is probably not the dominant failure mode on its own. This script fixes BOTH candidates anyway,
since they are not mutually exclusive and the storm-level fix is cheap and principled regardless:

WHAT CHANGES FROM v32:
  1. land_weight() now computes a STORM-normalized weight: every window's weight is
     PER_STORM_W / (number of that storm's training windows in the same bucket), so every
     distinct storm contributing to a bucket gets the SAME total sampling mass, regardless of
     whether it contributed 1 window or 78. This directly targets explanation (a): a storm with
     many correlated windows near land can no longer dominate the sampler just by having more
     windows than a storm passing land only briefly.
  2. Dosage cut sharply: PER_STORM_MTN=42, PER_STORM_FLAT=19 targets ~15%/~7% effective per-epoch
     share (verified locally), vs v32's ~48%/~9.5% -- targets explanation (b), leaving the
     open-ocean majority far more training exposure within the same early-stopping budget.
  3. A_MAX set to 1.0 (between v31's 0.65 and v32's 1.5) -- a moderate middle ground rather than
     compounding a large correction cap with a large dosage change; isolates what the storm-level
     fix itself buys before pushing a_max further in either direction.
  4. Nothing else. Same along-track-only LandDrag architecture -- _landtest_cross.py found no
     cross-track or asymmetry signal, so still not adding those.
...
N_SEEDS = int(os.environ.get("V33_SEEDS", "5"))
USE_LAND = int(os.environ.get("V33_USE_LAND", "1"))     # 0 = ablation, v23 with the same code path
A_MAX = float(os.environ.get("V33_AMAX", "1.0"))          # moderate middle ground, see docstring
TAG = os.environ.get("V33_TAG", "v33" if USE_LAND else "v33abl")
# storm-normalized weight budgets: PER_STORM_W is the TOTAL sampling mass a single storm gets in
# that bucket, split evenly across however many of that storm's windows fall in the bucket -- see
# land_weight() and the module docstring. Targets ~15%/~7% effective per-epoch share (verified
# locally against the actual training-set storm counts), vs v32's ~48%/~9.5%.
PER_STORM_MTN = float(os.environ.get("V33_PER_STORM_MTN", "42.0"))
PER_STORM_FLAT = float(os.environ.get("V33_PER_STORM_FLAT", "19.0"))
...
def land_weight(idx, chunk=4000):
    """STORM-normalized train sample weight -- the v33 fix over v32's per-window weighting.

    v32 weighted every window NEAR_W/MOUNTAIN_W independent of how many OTHER windows the same
    storm contributed to that bucket, so a storm with 78 correlated near-Taiwan windows got 78x
    the gradient mass of a storm passing land only once. Here every window's weight is
    PER_STORM_W / (that storm's window count in the SAME bucket), so every distinct storm gets
    the SAME total sampling mass regardless of how many windows it contributed -- a storm-count
    audit (see module docstring) found 640/658 distinct storms in the mountain/flat buckets, so
    this mostly changes DOSAGE EVENNESS across storms rather than adding storms that weren't
    already there, but it removes the specific pseudo-replication failure mode a from window-count
    weighting.

    Chunked for the same memory reason as v32: land_features_np broadcasts every row against all
    12,660 land cells at once; the full ~153k-row training set in one shot allocates tens of GB
    of intermediates, enough to OOM even Colab's High-RAM runtime."""
    n = len(idx)
    near = np.zeros(n, bool); mountain = np.zeros(n, bool)
    for i in range(0, n, chunk):
        ...
        near[i:i + chunk] = lok.astype(bool) & (dist_ahead <= 500.0)
        mountain[i:i + chunk] = near[i:i + chunk] & (terr_ahead >= MOUNTAIN_M)
    flat = near & ~mountain
    sid_idx = sid[idx]
    w = np.ones(n, "float64")
    for bucket_mask, per_storm_w in ((mountain, PER_STORM_MTN), (flat, PER_STORM_FLAT)):
        if not bucket_mask.any():
            continue
        bucket_storms = sid_idx[bucket_mask]
        uniq, counts = np.unique(bucket_storms, return_counts=True)
        count_of = dict(zip(uniq, counts))
        n_win_per_row = np.array([count_of[s] for s in bucket_storms])
        w[bucket_mask] = per_storm_w / n_win_per_row
    return w
...
with torch.no_grad():
    ...
    assert not _unexp, f"unexpected keys loading v23 into v33: {list(_unexp)[:5]}"
    ...
    print(f"init check: land off max|v33 - v23| = {_d1:.2e} (v33 starts as exactly v23)")
    print(f"init check: land on  max|v33 - v23| = {_d2:.2e} (land path is live)", flush=True)
...
_n_upweighted = int((TR_WEIGHT != 1.0).sum())
_total_mass = TR_WEIGHT.sum()
_sid_tr = sid[tr_idx]
_mtn_mask = TR_WEIGHT > (PER_STORM_FLAT / 1.0)  # crude split point between flat/mountain weight ranges
print(f"train sampler (storm-normalized): {_n_upweighted:,}/{len(tr_idx):,} windows reweighted, "
      f"PER_STORM_MTN={PER_STORM_MTN:.0f} PER_STORM_FLAT={PER_STORM_FLAT:.0f}", flush=True)
print(f"  effective per-epoch share: baseline {100*TR_WEIGHT[TR_WEIGHT==1.0].sum()/_total_mass:.1f}% "
      f"| upweighted (near-land) {100*TR_WEIGHT[TR_WEIGHT!=1.0].sum()/_total_mass:.1f}%", flush=True)
