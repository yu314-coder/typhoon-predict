...
V3 = build_v3()
zc = np.load("track_build/track_windows_v3data_maskpre80.npz", allow_pickle=True)
...
m6 = load(V3, "track_build/track_v7_best.pt")
print("\n=== v7 (expanded data, pre-1980 intensity masked) on full-20-lead 2020+ ===")
...
print(f"\nWP track: v7 {perr6.mean():.1f} vs v3(rerun) {perr3.mean():.1f} | diff {diff:+.1f} km")
print(f"storm-bootstrap (v7-v3): mean {diffs.mean():+.1f}, 95% CI [{lo:+.1f},{hi:+.1f}], P(v7 better)={(diffs<0).mean():.3f}")
