"""Predicted mean forecast vs real vmax/pressure for v10, v23, v34 on Typhoon Tip (1979) --
Tip is the most intense tropical cyclone ever recorded (870 hPa), so this is a genuine stress
test of extreme-intensity forecasting, not just an average-case check.

Writes track_build/tip_intensity.json (per-lead real mean, predicted mean, bias, MAE, corr for
each model) for make_tip_intensity_chart.py to draw.
"""
import json, re, math, os, numpy as np, torch, torch.nn as nn, torch.nn.functional as F, glob
...
print(f"v10: 1 | v23: {len(M23)} | v34: {len(M34)} seeds")
...
def predict(tag):
    ...
    for i in range(0, len(K), 64):
        j = K[i:i + 64]
        if tag == "v10":
            sv, _ = m10(torch.from_numpy(tr[j]), torch.from_numpy(vp[j]))
        else:
            h, hv = build_hist(j)
            ...
            if tag == "v34":
                ...
                sv = torch.stack([m(*a)[0] for m in M34]).mean(0)
            else:
                sv = torch.stack([m(*a)[0] for m in M23]).mean(0)
...
for tag in ("v10", "v23", "v34"):
    P = predict(tag)
    r = {}
    for ci, nm in [(2, "vmax"), (3, "pressure")]:
        real, pred = [], []
        for L in range(20):
            m = Kmask[:, L, ci]
            real.append(float(T[m, L, ci].mean()) if m.any() else None)
            pred.append(float(P[m, L, ci].mean()) if m.any() else None)
        r[nm] = {"real": real, "pred": pred}
    res[tag] = r
    print(f"{tag}: vmax real {[round(x,1) if x else None for x in r['vmax']['real']][::4]} | "
          f"pred {[round(x,1) if x else None for x in r['vmax']['pred']][::4]}")

json.dump(res, open("track_build/tip_intensity.json", "w"))
print("\nwrote track_build/tip_intensity.json")
