"""Standalone track export for any v17-architecture checkpoint set (v17, v18, ...) -- runs in the Colab TERMINAL on CPU, so it does not touch the
...
import sys
TAG = sys.argv[1] if len(sys.argv) > 1 else "v17"      # v17 | v18 | ...
NS = int(sys.argv[2]) if len(sys.argv) > 2 else 8
DRV = "/content/drive/MyDrive/typhoon"
CK = [f"{DRV}/{TAG}_seed{i}.pt" for i in range(NS)] + [f"/content/{TAG}_seed{i}.pt" for i in range(NS)]
CK = sorted(set(c for c in CK if os.path.exists(c)))
# prefer /content copies when both exist, they are identical but local
_seen = {}
for c in CK:
    k = os.path.basename(c)
    if k not in _seen or c.startswith("/content/"): _seen[k] = c
CK = [_seen[k] for k in sorted(_seen)]
CK = [c for c in CK if os.path.exists(c)]
assert CK, f"no {TAG} checkpoints found"
M = []
for c in CK:
    m = Net(); m.load_state_dict(torch.load(c, map_location="cpu", weights_only=False)["model"])
    m.eval(); M.append(m)
print(f"{TAG}: {len(M)} seeds on CPU", flush=True)
...
for s, nm in STORMS:
    ...
    print(f"{nm:11s} {len(k):3d} windows | {TAG} mean 120h {err:6.0f} km", flush=True)

json.dump(tracks_out, open(f"/content/{TAG}_tracks.json", "w"))
json.dump(series_out, open(f"/content/{TAG}_series.json", "w"))
print(f"wrote /content/{TAG}_tracks.json and /content/{TAG}_series.json")
