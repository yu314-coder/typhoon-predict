"""Export v23/v31/v33 tracks for the map (Wayne, Co-may, Hinnamnor, Bavi), gated on reproducing
the Colab ensemble numbers first, then adding Bavi (2026182N09163) which was never in
v23/v31_tracks.json before -- it's in track_windows_v13.npz (81 windows, WP), so it uses the
same in-dataset export path as the other three, not a hand-built pipeline like Noul/Tip.

v33 = v31/v32's exact LandDrag architecture, storm-normalized oversampling, a_max=1.0. RESULT
(Colab, 5 seeds): full WP+EP 442.33 km vs v23 434.96 km (+7.37, worse but the closest of any
land-drag attempt), WP-only 473.16 km. See land-interaction-lever memory: the aggregate
improvement over v31/v32 comes from open-ocean/flat-near-land, NOT the mountainous-near-land
bucket the whole exercise targeted -- drawn anyway since that's the honest picture to show.

Nothing is written until the local ensemble reproduces both Colab numbers (v31 443.07 km, v33
442.33 km, full WP+EP).
"""
import json, re, math, os, sys, glob, numpy as np, torch, torch.nn as nn, torch.nn.functional as F

torch.set_num_threads(8)
R = 111.2; KM6H = 6 * 3600 / 1000.0
EXPECT = {"v31": {"full": 443.07, "wp_only": 473.38}, "v33": {"full": 442.33, "wp_only": 473.16}}
...
TrackFormerLand31 = build_land_cls(0.65)
TrackFormerLand33 = build_land_cls(1.0)
...
MS = {"v31": load(TrackFormerLand31, sorted(glob.glob("downloads/v31ck/**/v31_seed*.pt", recursive=True))),
...
print(f"v31: {len(MS['v31'])} | v33: {len(MS['v33'])} | v23: {len(MS['v23'])} seeds")
...
# ---- MERGE into existing track_build/{tag}_tracks.json, never overwrite -- Tip and Noul were
# added there separately (by _tip_v23v31.py / _noul_predict.py) and must survive this run.
...
print("\nmerged Bavi + v33 into track_build/{v31,v33,v23}_tracks.json (Tip/Noul preserved)")
