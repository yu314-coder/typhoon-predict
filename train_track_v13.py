...
CKPT = os.environ.get("TRACK_CKPT", "track_build/track_v13_best.pt")
...
SLP = np.load("track_build/slp_patches.npy")                      # [N,2,17,17] surrounding pressure
SLP = (SLP.astype("float32") / np.array([5.0, 3.0], dtype="float32")[None, :, None, None])  # ~unit scale
...
class TrackFormerV9(nn.Module):
    def __init__(self, d=256, heads=8, ffn=1024, leads=20, dropout=0.2):
        ...
        self.steer_cnn = nn.Sequential(
            ...
            nn.Conv2d(64, d, 3, stride=2, padding=1), nn.GELU())
