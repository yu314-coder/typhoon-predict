...
## Published Pressure-Map Forecast Stage

The Trackformer 1.2 checkpoint is the route/intensity model. Future gridded
weather fields are generated by the separately published **WeatherNext
Cyclones Mini** pressure-map stage in [`pressure_map/`](pressure_map/). This
stage is now included so users do not have to fall back to the old saved
pressure-map artifacts.

Install the pinned upstream runtime:

```bash
python -m pip install -r pressure_map/requirements.txt
```

Run a causal 120-hour pressure-map forecast:

```bash
python pressure_map/run_pressure_map_forecast.py \
  --weights '/path/to/WeatherNextCyclones_Mini_<2024.npz' \
  --template /path/to/weathernext_template.nc \
  --initial-state /path/to/analysis_tminus6_t0.nc \
  --output pressure_rollout \
  --steps 20 \
  --members 1
```

The WeatherNext code and public sample/weight locations are documented in the
[Google DeepMind WeatherNext repository](https://github.com/google-deepmind/weathernext).
The checkpoint and sample template are external downloads; they are not
silently fetched by this repository.

The `--initial-state` file must contain exactly two matching analysis frames:
**t-6 h and t0**. The runner copies only the model's causal input variables
from those two frames, sets target-only template values to `NaN`, and rejects
coordinate mismatches. A GFS `f000` analysis may be used as an initialization
when legally available, but a positive-lead GFS/GEFS forecast may not be used.
The script never downloads or consumes JMA, JTWC, ECMWF, GFS, or GEFS forecast
products, and it does not use future observations.

The command writes:

| File | Contents |
|---|---|
| `predictions_selected.nc` | selected generated fields in WeatherNext coordinates |
| `pressure_maps.npz` | compact Pacific maps, shape `[members, lead, field, lat, lon]` |
| `pressure_map_manifest.json` | hashes, issue time, grid, fields, and causal audit |

The compact packet covers `0-45N, 100-220E` at a 2-degree stride and includes
MSLP, SST, 10-m winds, 850/500/200-hPa winds, 500-hPa height, cyclone risk,
pressure/wind/RMW signals, and R34/R50/R64 quadrant-radius signals. Multiple
members are independent samples from this same WeatherNext model; they are
not a mixture of old Trackformer versions or official forecasts. Official
agency forecasts may be drawn as comparison overlays only, after the model
packet is frozen.
