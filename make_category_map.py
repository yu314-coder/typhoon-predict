"""Real historical track for the six storms this project has validated against, drawn on a map
with each fix colored by intensity category (TD/TS/C1-C5, JTWC 1-min-sustained-wind thresholds in
knots) -- not model output, the actual recorded storm history, so this is ground truth.

Categories (kt, 1-min sustained, matching this project's vmax units throughout):
  TD  < 34      TS  34-63      C1  64-82      C2  83-95
...
# ---- real data per storm: (name, year, [(lat, lon, vmax_kt), ...]) ----
STORMS = []
...
for s, nm, yr in [("1986228N19120", "Wayne", "1986"), ("2025203N20124", "Co-may", "2025"),
                   ("2022239N22150", "Hinnamnor", "2022"), ("2026182N09163", "Bavi", "2026")]:
    k = np.where(sid == s)[0]
    k = k[np.argsort(bt[k])]
...
# ---- Tip 1979: real record, track_build/tip_fixed.npz ----
...
# ---- Noul 2026: hand-compiled real track (see _noul_v33.py for full sourcing) ----
KT_PER_MPH = 0.868976
NOUL_RAW = [
    (17.4, 128.4, 30), (18.0, 126.9, 35), (18.3, 125.1, 40), (18.7, 123.5, 45),
    (18.8, 121.8, 50), (19.7, 120.4, 70), (20.0, 119.3, 75), (20.8, 118.3, 80),
    (21.3, 116.6, 85), (21.8, 115.9, 90), (22.4, 115.1, 100), (22.9, 114.5, 85),
    (23.5, 114.1, 68), (24.4, 113.7, 57),
]
noul_pts = [(la, lo, w * KT_PER_MPH) for la, lo, w in NOUL_RAW]
STORMS.append(("Noul", "2026", noul_pts))
...
def panel(nm, yr, pts, W=460, H=360):
    ...
    while g <= lo1:
        ...
        g += step
    # thin connecting line, then category-colored dots on top
...
<title>Real storm tracks by intensity category (TD-C5)</title>
<style>
...
<div class="wrap">
 <header>
  ...
  <h1>Six real storms, colored by intensity category</h1>
  <p class="lede">Every fix is the storm's actual recorded position and wind speed &mdash; ground
  truth, the same real history used to score every model in this project. Color marks the
  JTWC/Saffir-Simpson category at that moment (1-min sustained wind, knots). Hollow ring marks
  genesis; peak intensity and its category are labeled on each panel.</p>
  <div class="legend">{legend}</div>
 </header>
 <div class="grid">{cards}</div>
 <footer>
  <p>Wayne (1986), Co-may (2025), Hinnamnor (2022), and Bavi (2026) are reconstructed from this
  project's own training windows (real IBTrACS-derived vmax at each issue time). Tip (1979) is from
  the dedicated real 3-hourly record used for its out-of-training test. Noul (2026) is hand-compiled
  from live public sources (Wunderground, Zoom Earth, PredictWind, HKO) since it postdates every
  dataset used elsewhere in this project.</p>
 </footer>
</div>"""

os.makedirs("paper", exist_ok=True)
open("paper/storms_by_category.html", "w").write(HTML)
print(f"wrote paper/storms_by_category.html ({len(HTML)/1000:.0f} KB)")
