...
const popup=p=>`${{p.time||''}}<br>lat ${{p.lat.toFixed(2)}} · lon ${{p.lon.toFixed(2)}}${{p.lead_hours===undefined?'':`<br>lead +${{p.lead_hours}} h`}}${{p.vmax_kt===undefined?'':`<br>wind ${{Number(p.vmax_kt).toFixed(0)}} kt`}}${{p.pressure_hpa===undefined||p.pressure_hpa===null?'':`<br>pressure ${{Number(p.pressure_hpa).toFixed(0)}} hPa`}}`;
