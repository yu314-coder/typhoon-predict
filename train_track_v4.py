"""TrackFormer v4 — v3 + Frenet-frame smooth-basis track residual + EMA weights (MPS).

Upgrades over v3 (protected dual-stream + persistence-residual):
  * FRENET SMOOTH-BASIS residual. Instead of 20 free 2-D per-step residuals, the track head emits
    K=8 DCT coefficients per axis in the storm-relative Frenet frame (along-track / cross-track of the
    current heading). The residual curve is  a_l = sum_k c^a_k B_{l,k},  c_l = sum_k c^c_k B_{l,k};
    residual = a_l * u_hat + c_l * n_hat, added to the damped-persistence baseline. This decouples
    speed change (along) from turning/recurvature (cross), enforces smoothness, and cuts the effective
    track output dimension 40 -> 16 (regularization for the overfit-prone track head).
  * EMA of parameters (decay 0.999); validation + checkpoint use the EMA weights (generalization boost).
  * Everything else (dual encoders, gradient routing, zero-init gated thermo->track adapter, intensity
    decoder, losses) is identical to v3 so the comparison is clean.
"""
...
CKPT = os.environ.get("TRACK_CKPT", "track_build/track_v4_best.pt")
...
class TrackFormerV4(nn.Module):
    def __init__(self, d=256, heads=8, ffn=1024, leads=20, dropout=0.2, K=K_BASIS):
        ...
        self.register_buffer("B", dct_basis(leads, K))          # [L,K]
        ...
        # smooth-basis Frenet coefficient head: 2*K (along + cross), zero-init -> starts as pure persistence
        self.coef = nn.Linear(d, 2 * K); nn.init.zeros_(self.coef.weight); nn.init.zeros_(self.coef.bias)
    ...
    def forward(self, track, v0_raw):
        ...
        # smooth-basis Frenet residual
        coef = self.coef(h_track.mean(1)).view(b, 2, self.K)               # [b,2,K]
        along = coef[:, 0] @ self.B.t()                                    # [b,L]
        cross = coef[:, 1] @ self.B.t()                                    # [b,L]
        speed = v0_raw.norm(dim=1, keepdim=True).clamp(min=1e-3)           # [b,1]
        u = v0_raw / speed                                                 # heading unit [b,2]
        nrm = torch.stack([-u[:, 1], u[:, 0]], dim=1)                      # left-normal [b,2]
        residual = along.unsqueeze(-1) * u.unsqueeze(1) + cross.unsqueeze(-1) * nrm.unsqueeze(1)  # [b,L,2] normalized
        base = (self.rho.view(1, self.leads, 1) * v0_raw.unsqueeze(1)) / 100.0
        motion = base + residual
...
print("\nTrackFormer v4 (Frenet smooth-basis + EMA). v3 baseline WP: track 659.0/pres 18.06/radius 28.83; "
      "all-basin: track 592.1")
