"""TrackFormer v10.1 — v10 architecture, 72% more training data (1950+), plus ONI.

v10 uses no reanalysis fields at all, so extending the record back to 1950 costs only an
IBTrACS rebuild -- no SLP or wind downloads. Training windows go 153,378 -> 264,454 while
valid and test stay byte-identical, so the test number is directly comparable to v10 (549.3)
and v17 (462.8).

ONI (feature 54) is the Nino 3.4 SST anomaly: a basin-scale ENSO signal that shifts where WP
typhoons form and how far they recurve, and that no storm-centred patch can see.

CAVEAT: pre-satellite best-track data is less accurate. WP had aircraft reconnaissance from
1945-1987 so intensities are better than one might fear, but 1950s-60s positions carry more
error than modern fixes. If this run does not beat v10, the next thing to try is 1970+ rather
than assuming more data is always better.
...
NPZ = os.environ.get("TRACK_NPZ", "track_build/track_windows_v20.npz")
CKPT = os.environ.get("TRACK_CKPT", "track_build/track_v20_best.pt")
...
ENV_COLS = [48, 49, 50, 51, 52, 53, 54]   # + ONI
