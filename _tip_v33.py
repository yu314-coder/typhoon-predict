...
def build_land_cls(a_max):
    g = {"V23": V23, "torch": torch, "nn": nn, "F": F, "math": math, "G": G, "A_MAX": a_max, "USE_LAND": 1}
    exec(land_src, g); exec(tfl_src, g)
    return g["TrackFormerLand"]


TrackFormerLand = build_land_cls(0.65)      # v31
TrackFormerLand33 = build_land_cls(1.0)     # v33
...
M33 = [load(f, TrackFormerLand33) for f in sorted(glob.glob("downloads/v33ck/**/v33_seed*.pt", recursive=True))]

print(f"steering inputs differ: {float(np.abs(S17 - S20).max())} max abs diff")
print(f"models: v21 {len(M21)} | v20 {len(M20)} | v17 {len(M17)} | v23 {len(M23)} | v31 {len(M31)} | v33 {len(M33)} seeds")
...
print(f"  v33   (+land-drag storm)   {full_storm(M33, S20, TrackFormerLand33, use_hist=True, use_land=True):8.2f} km")
...
for tag, ms, cls, use_land in (("v23", M23, V23, False), ("v31", M31, TrackFormerLand, True),
                                ("v33", M33, TrackFormerLand33, True)):
