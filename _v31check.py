"""Local (CPU) init-assertion check for v31 (TrackFormerLand), sourcing every class from LOCAL
files instead of urllib/GitHub -- same pattern as _v29check.py. Verifies:
  1. land off  -> v31 output EXACTLY equals v23's (max diff 0)
  ...
  5. state-dict remap loads v23 weights into v31 with only the new `land.*` keys missing
"""
...
assert _d1 < 1e-5, f"v31 does not reduce to v23 at init: {_d1}"
print(f"OK: land OFF max|v31 - v23| = {_d1:.2e} (v31 starts as exactly v23)")
...
print(f"OK: land ON  max|v31 - v23| = {_d2:.2e} (land path is live)")
...
print("\nALL CHECKS PASSED -- v31 (TrackFormerLand) is ready for Colab training")
